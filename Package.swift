...
#if os(Linux)
let msquicTarget: Target = .binaryTarget(
    name: msquicTargetName,
    path: "\(msquicTargetName).artifactbundle"
)
#else
let msquicTarget: Target = useDebugMsQuic
    ? .binaryTarget(
        name: "MsQuicDebug",
